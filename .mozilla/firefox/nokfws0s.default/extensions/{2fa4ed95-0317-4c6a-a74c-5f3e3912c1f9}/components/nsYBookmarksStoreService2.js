...
var grscDeliciousFeedsRoot = gRdfService.GetResource("NC:DeliciousFeedsRoot");
...
var grscDelFeedUrl = gRdfService.GetResource(gNC_NS + "DelFeedURL");
var grscDelFeedLastUpdate = gRdfService.GetResource(gNC_NS + "DelFeedLastUpdate");
var grscDelFeedType = gRdfService.GetResource(gNC_NS + "DelFeed");
...
YBookmarksStoreService.prototype = {
  ...
  _getBookmarkRoot : function() {
   ...
   _getDeliciousFeedsRoot : function() {
    if (! gRdfContainerUtils.IsSeq(this._fileDatasource, grscDeliciousFeedsRoot)) {
      yDebug.print("No longer a seq: " + grscDeliciousFeedsRoot.Value);
    }
    return this._DeliciousFeedsRoot;
  },
  ...
  _initRoots : function() {
    var result = false;      
    if (! gRdfContainerUtils.IsSeq(this._fileDatasource, grscBookmarksRoot)    
      || ! gRdfContainerUtils.IsSeq(this._fileDatasource, grscDeliciousFeedsRoot)
    ...
    this._transactionRoot = gRdfContainerUtils.MakeSeq(this._fileDatasource,
      grscTransactionRoot);
    this._DeliciousFeedsRoot = gRdfContainerUtils.MakeSeq(this._fileDatasource,
      grscDeliciousFeedsRoot); 
    ...
    if (! gRdfContainerUtils.IsSeq(this._fileDatasource, grscTopRoot)) {
      ...
      topRoot.AppendElement(grscDeliciousFeedsRoot);
      result = true;
    }
  ...
  _updateLivemarks : function() {
    yDebug.print("Livemarks being updated...", YB_LOG_MESSAGE);
    ...
    //Take refresh interval from prefs and use it
    var prefService = Components.classes["@mozilla.org/preferences-service;1"]
		                             .getService(Components.interfaces.nsIPrefBranch);
    var interval = prefService.getIntPref("extensions.ybookmarks@yahoo.livemarkUpdate.interval");

    if(!interval) interval = 5; //keep 5 minutes as default
    
    this._hiddenWindow.setTimeout(function (self) { self._updateLivemarks(); },
      interval * 60 * 1000, this);
  },
  ...
  //Helper function to map sort order string to sqlite number convention.
  _getSortOrderFromString : function(order) {
    switch(order){
        case FAVTAG_ORDER_CHRONO :   return "0";
        case FAVTAG_ORDER_CHRONO_REVERSE :   return "1";
        case FAVTAG_ORDER_ALPHANUM :   return "2";
        case FAVTAG_ORDER_ALPHANUM_REVERSE :   return "3";
        default : return 1;
    }
  },
  //Another helper function for import to RDF
  getTransactionProperties : function(transaction) {    
    try {
        var transactionType = transaction.getProperty( "transactionType" );
        var obj = {};    
        if(transactionType) {
            obj.transactionType = transactionType;
        }    
        
        if(transactionType.indexOf("deleteBookmark") != -1) {
            var bkProps = ["url"];
        } else if(transactionType.indexOf("Bookmark") != -1) { //Add/Edit Bookmarks
            var bkProps = ["title", "url", "tags", "notes", "shared", "rss",
                           "localOnly", "shortcut", "postData", "microsummary"];        
        } else if(transactionType.indexOf("setBundle") != -1) {
            var bkProps = ["name", "url", "tags"];
        } else if(transactionType.indexOf("deleteBundle") != -1) {
            var bkProps = ["name", "url"];
        }
        
        var len = bkProps.length;
        for(var i = 0; i < len; ++i) {
            try {
                obj[bkProps[i]] = null;
                obj[bkProps[i]] = transaction.getProperty( bkProps[i] );                
            } catch(e) {
                //yDebug.print("getTransactionProperties::exception in prop access:" + e, YB_LOG_MESSAGE);
            }    
        }            
        return obj;
    } catch(e) {
        yDebug.print("getTransactionProperties::exception:" + e, YB_LOG_MESSAGE);
    }
  },
  ...
  _init : function(filename) {
    ...
    */   
    ...
    try {
        //var profiler = Components.classes["@yahoo.com/YJSProfiler;1"].
		//		      getService(Components.interfaces.nsIYJSProfiler);
        //var blockId = profiler.startProfileBlock("MigrateRDFData");				      
        ...
        if(!lastUpdateTime) {
            yDebug.print("RDFStore: Error accessing lastUpdate time from RDF", YB_LOG_MESSGE);
            //return;
        } else {
            yDebug.print("RDFStore: Setting lastUpdate time:" + lastUpdateTime, YB_LOG_MESSAGE);
            this.sqliteStore.setLastUpdateTime(lastUpdateTime);
        }        
        
        /**
         * add following methods to timeout
         * so that FF UI can load
         */
         var pref = Components.classes["@mozilla.org/preferences-service;1"].
				             getService(Components.interfaces.nsIPrefBranch);
	     pref.setBoolPref("extensions.ybookmarks@yahoo.localstore.importingFromRDF", true);

         this._hiddenWindow.setTimeout( function (self) {
            try {
                var os = Components.classes["@mozilla.org/observer-service;1"].
                      getService(Components.interfaces.nsIObserverService);
                os.notifyObservers(null, "ybookmark.migrationBegin", "");

                self.rdfBookmarks = self.getBookmarks( null, {} );
                
                /**
                 * Check if bookmarks are more than 2000 and show alert
                 */
                if(self.rdfBookmarks.length > 2000) {
                    os.notifyObservers(null, "ybookmark.showMigrationDialog", "");
                }
                   
                /**
                 * Restore fav tags, bundles and transactions first
                 */
                self.restoreFavTags();
                self.restoreBundles();
                ...
                /**
                 * start restoring bookmarks
                 */
                self.restoreBookmarks(self, 0);
            } catch(e) {
                yDebug.print("RDFStore: Incomplete Import, Exception:"  + e, YB_LOG_MESSAGE);
            }
        }, 2000, this);
        
        //profiler.endProfileBlock("MigrateRDFData", blockId);
    } catch(e) {
        yDebug.print("RDFStore: Incomplete Import, Exception:"  + e, YB_LOG_MESSAGE);
    }    
  },  
  
  restoreBundles : function() {
       // var profiler = Components.classes["@yahoo.com/YJSProfiler;1"].
		//		      getService(Components.interfaces.nsIYJSProfiler);
        //var blockId = profiler.startProfileBlock("restoreBundles");
    try {
        //bundles
        var bundles = this.getBundles({});
        var bundLen = bundles.length;
        for(var i = 0; i < bundLen; ++i) {
            var bundle = bundles[i];
            this.sqliteStore.setBundle({name:  bundle.name,
                                        tags:  bundle.tags,
                                        order: this._getSortOrderFromString(bundle.order)});
        }
    } catch(e) {
        yDebug.print("RDFStore::restoreBundles:: Exception:"  + e, YB_LOG_MESSAGE);
    }
   // profiler.endProfileBlock("restoreBundles", blockId);
  },
  
  restoreBookmarks : function(self, count) {
      //  var profiler = Components.classes["@yahoo.com/YJSProfiler;1"].
		//		      getService(Components.interfaces.nsIYJSProfiler);
      //  var blockId = profiler.startProfileBlock("restoreBookmarks");
    try {
        var bookmarks = self.rdfBookmarks;
        var bklen = bookmarks.length;
        
        //yDebug.print("RDFStore::restoreBookmarks:: FINISHED RESTORING BOOKMARKS FROM "+count, YB_LOG_MESSAGE);
        var prefs = Components.classes["@mozilla.org/preferences-service;1"].
				             getService(Components.interfaces.nsIPrefBranch);
        var chunkSize = 100;
        try {
            chunkSize = prefs.getIntPref("extensions.ybookmarks@yahoo.localstore.importingFromRdfChunk");
        } catch(e) {}
        
        if(count > bklen) {
            self.sqliteStore.setLastUpdateTime("-1"); //This is to force a periodic sync
            
            var os = Components.classes["@mozilla.org/observer-service;1"].
                  getService(Components.interfaces.nsIObserverService);
            os.notifyObservers(null, "ybookmark.migrationEnd", "");            
  		    prefs.setBoolPref("extensions.ybookmarks@yahoo.localstore.importingFromRDF", false);

            yDebug.print("RDFStore::restoreBookmarks:: FINISHED RESTORING BOOKMARKS", YB_LOG_MESSAGE);                  
            
            return;
        }
        var i = count;
        for(i = count; i < bklen; ++i) {            
            var book = self.getBookmarkFromResource(bookmarks[i]);         
            if(book.type.indexOf("Bookmark") != -1) {
                self.sqliteStore.addBookmarkObject(book);
            } else {
                if(book.tags) {
                    var jsTags = ybookmarksUtils.nsArrayToJs(book.tags);
                }
                var feedUrl = book.url;
                self.sqliteStore.addLivemark( feedUrl, book.title, book.description,
                                  jsTags ? jsTags.length : 0, jsTags,
                                  book.shared, book.localOnly);
                self.sqliteStore.setStringPropertyForBookmark(feedUrl, "meta_hash", book.meta_hash);
                self.sqliteStore.setStringPropertyForBookmark(feedUrl, "hash", book.hash);                  
            }
           ...
           if(i != 0 && i % (chunkSize*2) == 0) {
                ...
                var subject = { start: i, chunk: chunkSize, total: bklen };
                subject.wrappedJSObject = subject;
                
                os.notifyObservers(subject, "ybookmark.migrationInProgress", "");
           }
        ...
        //By default wait time is 6 seconds
        var MIGRATION_CHUNK_WAIT = 6000;
        try {
            MIGRATION_CHUNK_WAIT = prefs.getIntPref("extensions.ybookmarks@yahoo.localstore.importingFromRdfWait");
        } catch(e) {}
        
        self._hiddenWindow.setTimeout(self.restoreBookmarks, MIGRATION_CHUNK_WAIT, self, ++i);
        
    } catch(e) {
        yDebug.print("RDFStore::restoreBookmarks:: Exception:"  + e, YB_LOG_MESSAGE);
    }
       // profiler.endProfileBlock("restoreBookmarks", blockId);
  },
    
  restoreFavTags : function() {
      //  var profiler = Components.classes["@yahoo.com/YJSProfiler;1"].
	//			      getService(Components.interfaces.nsIYJSProfiler);
     //   var blockId = profiler.startProfileBlock("restoreFavTags");

    try {
        var favTags = this.getFavoriteTags([], {});        
        var favLen = favTags.length;
        yDebug.print("RDFStore: Favtags length:"  + favLen, YB_LOG_MESSAGE);
        if(favLen) {
            for(var i = 0; i < favLen; ++i) {
                var order = FAVTAG_ORDER_DEFAULT;
                order = this.getFavoriteTagOrder(favTags[i]);                
                yDebug.print("Favtags:"  + i + "Name:" +  favTags[i] + " Order:" + order + " sortOrder:" + this._getSortOrderFromString(order), YB_LOG_MESSAGE);                
                var favTag = ybookmarksUtils.sanitizeFavtag(favTags[i]);
                if(favTag) {
                    this.sqliteStore.addFavoriteTag(favTag);
                    this.sqliteStore.setFavoriteTagOrder(favTag, this._getSortOrderFromString(order));                
                }
            }
            this.sqliteStore.saveFavoriteTags();
        }
    } catch(e) {
        yDebug.print("RDFStore::restoreFavTags:: Exception:"  + e, YB_LOG_MESSAGE);
    }
    //profiler.endProfileBlock("restoreFavTags", blockId);
  },
  
  restoreTransactions : function() {
        //var profiler = Components.classes["@yahoo.com/YJSProfiler;1"].
		//		      getService(Components.interfaces.nsIYJSProfiler);
        //var blockId = profiler.startProfileBlock("restoreTransactions");

    try {
        var transactionQueue = this.getTransactions();
        ...
        while (transactions.hasMoreElements()) {
            ...
            if(transactionState != 2) {              
                var obj = this.getTransactionProperties(transaction);
                yDebug.printObject(obj, "Transaction");
                if(obj) {
                    var tags = obj.tags ? obj.tags : null;
                    if(tags) {                        
                        obj.tags = new Array();
                        obj.tags = tags.split(" ");                        
                    }
                    ...
                    if( (obj.transactionType.indexOf("addBookmark") != -1) ||
                        (obj.transactionType.indexOf("editBookmark") != -1) ) { //Bkmk or Lvmk
                        if(obj.rss) {
                            this.sqliteStore.addBookmark( obj.url, obj.title,
                                                  obj.notes, 
                                                  obj.shortcut,
                                                  obj.postData,
                                                  obj.tags ? obj.tags.length : 0, obj.tags, 
                                                  obj.shared,
                                                  obj.localOnly);
                            
                        } else {
                            this.sqliteStore.addLivemark( obj.url, obj.title,
                                               obj.notes,
                                               obj.tags ? obj.tags.length : 0, obj.tags, 
                                               obj.shared,
                                               obj.localOnly);
                        }
                        this.sqliteStore.addTransaction(obj.transactionType, 0, "bookmark", obj.url);
                    } else if(obj.transactionType.indexOf("setBundle") != -1) {
                        obj.tags = ybookmarksUtils.jsArrayToNs(obj.tags);
                        this.sqliteStore.setBundle({name:  obj.name,
                                        tags:  obj.tags,
                                        order: this._getSortOrderFromString(obj.order)});
                        this.sqliteStore.addTransaction(obj.transactionType, 0, "bundle", obj.name);
                    }
                }
            }
        }
    } catch(e) {
        yDebug.print("RDFStore::restoreTransactions Exception:" + e, YB_LOG_MESSAGE);
    }
   // profiler.endProfileBlock("restoreTransactions", blockId);
  },
  ...
  getBookmarkFromResource : function (aBookmarkResource) {
    ...
    try {
      ...
      var urlHash = this._getTargetLiteral(aBookmarkResource, grscHash);
      var urlMetaHash = this._getTargetLiteral(aBookmarkResource, grscMetaHash);
  ...
  getTagSuggestions : function(aKeyword, keepExactMatch) {
    ...
    try {
      ...
      while (tags.hasMoreElements()) {
        ...
        if (tag) {
          tag = tag.toLowerCase();
          if (tag.indexOf(keyword) == 0 && (tag.length != keyword.length || keepExactMatch)) {
      ...
      yDebug.print("getTagSuggestions"+e, YB_LOG_MESSAGE);
    } finally {
