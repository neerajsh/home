...
;
; amd : strip foreground/background color from frame params and use
; set-XXX-color instead. Otherwise they don't get set at startup.
;

(defun amd-color-hack (old-list)
  (let (elem new-list)
    (dolist (elem old-list)
      (setq name (symbol-name (if (listp elem) (car elem) elem)))
      (cond
       ((string-equal "background-color" name) (set-background-color (cdr elem)))
       ((string-equal "foreground-color" name) (set-foreground-color (cdr elem)))
       (t (setq new-list (cons elem new-list)))))
    new-list))
...
Called from `color-theme-install'."
  (setq params (amd-color-hack (color-theme-filter
