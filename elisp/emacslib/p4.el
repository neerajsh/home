...
(defp4cmd p4-edit (show-output)
  ...
  ;; amd - use p4-buffer-file-name-special
  (let ((args (p4-buffer-file-name-special))
	refresh-after)
...
;; amd - modified to use define-prefix-command, so that autoload will find it
(define-prefix-command 'p4-prefix-map)
...
(define-key p4-prefix-map "\C-r" 'p4-resolve)
...
(define-key p4-prefix-map "V" 'p4-blame)
...
;; amd - only used by p4-edit and p4-revert
(defun p4-buffer-file-name-special ()
  (let ((f (p4-buffer-file-name)))
    (cond (f (if (string-match  ".*\\.el[c]?$" f)
		 (format "%s.el*" (file-name-sans-extension f))
	       f))
	  (t nil))))
